    ...
    <div className="mx-auto max-w-7xl space-y-6">
      ...
      <div className="grid gap-6 xl:grid-cols-[260px_minmax(0,1fr)]">
        <Card className="h-fit xl:sticky xl:top-6">
          <ol className="space-y-1">{STEPS.map((item, index) => { const number = index + 1; const active = number === step; const passed = number < step; return <li key={item.title}><button type="button" onClick={() => setStep(number)} className={`flex w-full items-center gap-3 rounded-lg px-3 py-3 text-left text-sm ${active ? 'bg-[#eef3f0] font-semibold text-[#496B5A]' : 'text-[#637079] hover:bg-[#F8FAFA]'}`}><span className={`flex size-7 shrink-0 items-center justify-center rounded-full text-xs font-bold ${active || passed ? 'bg-[#496B5A] text-white' : 'bg-[#EEF2F2]'}`}>{passed ? <Check size={14} /> : number}</span>{item.title}</button></li> })}</ol>
        </Card>
        ...
        <Card padding="lg">
          ...
          <div className="py-6">{renderStep(step, form, set, setConfig)}</div>
...
function renderStep(step, form, set, setConfig) {
  ...
  if (step === 3) return <div className="space-y-4"><Input label="Agency tagline" value={config.tagline} onChange={(e) => setConfig('tagline', e.target.value)} maxLength={120} /><Input label="Hero eyebrow" value={config.hero_eyebrow} onChange={(e) => setConfig('hero_eyebrow', e.target.value)} /><Input label="Main homepage headline" value={config.hero_title} onChange={(e) => setConfig('hero_title', e.target.value)} required /><Textarea label="Homepage introduction" rows={4} value={config.hero_subtitle} onChange={(e) => setConfig('hero_subtitle', e.target.value)} /></div>
  ...
  const labels = { agency_name: 'Agency name', public_email: 'Public email', phone: 'Public phone', about: 'Agency description', location: 'Address or service area', logo: 'Agency logo', cover_image: 'Homepage cover image', hero_title: 'Homepage headline', hero_subtitle: 'Homepage introduction', seo_title: 'SEO title', seo_description: 'SEO description' }
  return <div className="space-y-6"><div className={`rounded-xl border p-5 ${form.is_ready_to_publish ? 'border-emerald-200 bg-emerald-50' : 'border-amber-200 bg-amber-50'}`}><div className="flex items-start gap-3"><Globe2 className="mt-0.5 text-[#496B5A]" size={21} /><div><p className="font-semibold text-[#263238]">{form.is_website_published ? 'Your website is published' : 'Complete the publishing checklist'}</p><p className="mt-1 text-sm text-[#637079]">Save this step to refresh the completeness result before publishing.</p></div></div></div><div><h3 className="font-semibold text-[#263238]">Required information</h3><ul className="mt-3 grid gap-2 sm:grid-cols-2">{Object.entries(labels).map(([key, label]) => { const missing = (form.missing_fields || []).includes(key); return <li key={key} className="flex items-center gap-2 rounded-lg border border-[#DDE5E3] px-3 py-2 text-sm"><span className={`flex size-5 items-center justify-center rounded-full ${missing ? 'bg-amber-100 text-amber-700' : 'bg-emerald-100 text-emerald-700'}`}>{missing ? '!' : <Check size={12} />}</span>{label}</li> })}</ul></div></div>
}
