...
import { Bath, Bed, CalendarDays, Check, Heart, MapPin, Maximize2, Phone, UserRound } from 'lucide-react'
...
import { getCustomerSession, getSavedProperties, toggleSavedProperty, trackPublicPropertyEvent } from '@/services/publicService'
  ...
  return (
    <main className="mx-auto max-w-7xl space-y-10 px-4 py-8 sm:px-6">
      <div className="grid gap-3 overflow-hidden rounded-2xl lg:grid-cols-[2fr_1fr]">
        <Media image={media[0]?.file} title={property.title} className="h-[420px]" />
        <div className="grid grid-cols-2 gap-3 lg:grid-cols-1">{media.slice(1, 3).map((item) => <Media key={item.id} image={item.file} title={item.title || property.title} className="h-[204px]" />)}</div>
      </div>
