...
export default function PublicMapPage() {
  ...
  const query = usePublicProperties(agency.license_number, { location })
  if (query.isLoading) return <div className="py-20"><PageSpinner /></div>
  const properties = query.data || []; const focused = properties.find((item) => item.latitude && item.longitude)
