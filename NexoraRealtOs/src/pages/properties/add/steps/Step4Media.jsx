...
import { CloudUpload, X, ImageIcon, Star } from 'lucide-react'
import { cn } from '@/lib/cn'

export default function Step4Media({ files, onChange }) {
    ...
    <div className="space-y-4">
      ...
      >
        ...
        <div>
          ...
          <p className="mt-1 text-xs text-[#8b969d]">
            JPG, PNG, WEBP, MP4 — max 10MB each
          </p>
        </div>
        <button
          type="button"
      ...
      {files.length === 0 && (
        <div className="flex gap-2.5 rounded-xl bg-amber-50 border border-amber-200 p-3.5">
          ...
          <p className="text-xs text-amber-800 leading-relaxed">
            <span className="font-semibold">Pro Tip:</span> High-quality photos of your property can
            increase leads by up to 40%. Ensure your images are well-lit and showcase key areas
            such as the living room, kitchen, and bedrooms.
          </p>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 gap-4 pt-2">
        <div className="flex flex-col gap-1.5">
          <label className="text-sm font-medium text-[#263238]">Virtual Tour URL</label>
          <div className="relative flex items-center">
            <span className="absolute left-2.5 text-[#ef4444]">
              <span className="text-xs">●</span>
            </span>
            <input
              type="url"
              placeholder="https://yourtour.example.com/..."
              className="w-full h-10 rounded-lg border border-[#DDE5E3] bg-white pl-7 pr-3 text-sm text-[#263238] placeholder:text-[#8b969d] focus:outline-none focus:ring-2 focus:ring-[#496B5A]/30 focus:border-[#496B5A] transition-colors"
            />
          </div>
        </div>
        <div className="flex flex-col gap-1.5">
          <label className="text-sm font-medium text-[#263238]">Floor Plan Document</label>
          <input
            type="file"
            accept=".pdf,.jpg,.png"
            className="h-10 rounded-lg border border-[#DDE5E3] bg-white px-3 text-sm text-[#637079] file:mr-3 file:rounded-md file:border-0 file:bg-[#eef3f0] file:px-3 file:py-1 file:text-xs file:font-medium file:text-[#496B5A] cursor-pointer"
          />
        </div>
      </div>
    </div>
